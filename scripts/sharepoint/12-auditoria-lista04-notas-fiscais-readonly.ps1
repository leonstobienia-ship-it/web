...
function Resolve-ReadonlyList {
    ...
    try {
        return Get-PnPList -Connection $Connection -Identity $listGuid -Includes Title,Id,Hidden,ItemCount,RootFolder,Fields
    }
    catch {
        foreach ($title in $candidateListTitles) {
            try {
        ...
        throw "Lista 04 - Notas Fiscais Recebidas nao localizada por GUID ou nomes candidatos."
    }
}
...
$jsonPath = Join-Path $outputFullDirectory "lista04-notas-fiscais-fields-readonly.json"
$markdownPath = Join-Path $outputFullDirectory "lista04-notas-fiscais-fields-readonly.md"

$connection = Connect-PnPReadonly
$web = Get-PnPWeb -Connection $connection -Includes Url,Title
$list = Resolve-ReadonlyList -Connection $connection
...
Add-Line $lines "# Auditoria readonly Lista 04 - Notas Fiscais"
Add-Line $lines ""
...
Add-Line $lines ""
Add-Line $lines "## Pendencias para V2.7A.6"
Add-Line $lines ""
...
Write-Host "Auditoria readonly Lista 04 gerada:"
